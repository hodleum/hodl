...
class BlockUnittest(unittest.TestCase):
    ...
    def test_equal(self):
        ...
        b = bch[1].txs[1]
        b2.from_json(str(b))
        #print('x1==x2',b.sign==b2.sign, [x1[i]==x2[i] for i in range(len(x1))])
        print(b==b2, b.__dict__==b2.__dict__, 'keys')
        for k in b.__dict__.keys():
            if not b2.__dict__[k]==b.__dict__[k]:
                print(k, b2.__dict__[k], b.__dict__[k])
