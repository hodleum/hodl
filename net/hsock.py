...
class HSock(Thread):
    ...
    def __init__(self, sock=None, conn=None, addr='', myaddrs=tuple(), peers=Peers()):
        ...
        # todo: send first message generated by protocol.generate
    ...
    def send(self, data, t='none', request='', full=False, peers='from_HSock'):
        """
        ...
        for sock in self.socks:
            if sock:
                ...
                send(sock, generate(data, t, peers, [addr[1] for addr in self.myaddrs], "text", full, self.name))
    ...
    def recv_by_sock(self, sock):
        ...
        handle(self.in_msgs[-1], self.name, self.peers if 'peers' in self.__dict__.keys() else Peers())
